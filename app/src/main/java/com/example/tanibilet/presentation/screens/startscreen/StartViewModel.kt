...
@HiltViewModel
class StartViewModel @Inject constructor(application: Application): ViewModel(){
        ...
        init {
            ...
            if(_state.value.listOffer.isNullOrEmpty()) {
                viewModelScope.launch {
                    try {
                        ...
                        Log.e("userviewmodellength", _state.value.isLoading.toString())
                    } catch (e: IOException) {
        ...
        fun onEvent(event: StartScreenEvent){
            when(event){
                is StartScreenEvent.TakeInformationAboutAllTours->{
                    downloadInformationAboutAllTours()
                }
            }
        }


        fun downloadInformationAboutAllTours(){
            _state.value = _state.value.copy(
                isLoading = true
            )
            if(_state.value.listOffer.isNullOrEmpty()) {
